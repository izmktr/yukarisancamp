...
from .clan_score import ClanScore
...
from .reserve_unit import ReserveUnit
...
#            (['dtest'], self.DamageTest),
    ...
    def CheckInputChannel(self, message):
        if self.inputchannel is None or message.channel.name != INPUT_CHANNEL:
            return True
    ...
    async def AttackCheck(self, message, member : ClanMember, bidx : int):
        if self.CheckInputChannel(message):
            self.TemporaryMessage(message.channel, '%s のチャンネルで発言してください' % INPUT_CHANNEL)
            return True
    ...
    async def AttackPlan(self, message, member : ClanMember, opt : str):
        ...
        for m in self.members.values():
            ...
            # 3凸済み
            if MAX_SORTIE <= len(attackedplan):
            ...
            unknowncount = MAX_SORTIE - len(attackedplan) - len(remainplan)
...
#            text += n + '\n'
    ...
    def AllowMessage(self, message):
        if message.channel.name == INPUT_CHANNEL: return True
    ...
    def SetOutputChannel(self):
        if self.outputchannel is None:
            self.outputchannel = self.FindChannel(self.guild, OUTPUT_CHANNEL)

    def SetInputChannel(self):
        if self.inputchannel is None:
            self.inputchannel = self.FindChannel(self.guild, INPUT_CHANNEL)
    ...
    async def on_message(self, message):
        if self.AllowMessage(message):
            ...
            if self.outputchannel is None:
                await message.channel.send('%s というテキストチャンネルを作成してください' % (OUTPUT_CHANNEL))
                return False
    ...
    def StatusMemberList(self):
        ...
        fulllist : List[List[ClanMember]] = [[] for _i in range(MAX_SORTIE + 1) ]
