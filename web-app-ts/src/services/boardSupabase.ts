...
function parseDamageValue(value: unknown): number {
...
function normalizeDifficultyValue(value: unknown): number {
  if (typeof value === 'number' && Number.isFinite(value)) {
...
function parseBattleTimeToSeconds(value: unknown): number {
  const text = typeof value === 'string' ? value.trim() : '';
...
function parseBattleDateToIso(value: unknown): string {
  const text = typeof value === 'string' ? value.trim() : '';
  if (!text) {
...
export function parseTimelog(text: string): ParsedBoardTimelog {
  const lines = text.split(/\r?\n/);
  const result: ParsedBoardTimelog = {
    bossname: '',
    mode: '',
    damage: '',
    battleTime: '',
...
export async function getCurrentClanBattleYearMonth(): Promise<string> {
  const rows = await supabaseRequest<Array<{ yearmonth: string }>>('setting_clanbattle?select=yearmonth&id=eq.0&limit=1', {
    method: 'GET'
  });
...
export async function listCurrentMonthBoardPosts(): Promise<BoardSupabaseRow[]> {
  const yearmonth = await getCurrentClanBattleYearMonth();
  return await supabaseRequest<BoardSupabaseRow[]>(`board_posts?select=*&yearmonth=eq.${encodeURIComponent(yearmonth)}&order=created_at.desc`, {
    method: 'GET'
  });
}

export async function getCurrentMonthBoardPostByLegacyId(legacyId: string): Promise<BoardSupabaseRow | null> {
  const yearmonth = await getCurrentClanBattleYearMonth();
  const rows = await supabaseRequest<BoardSupabaseRow[]>(`board_posts?select=*&legacy_id=eq.${encodeURIComponent(legacyId)}&yearmonth=eq.${encodeURIComponent(yearmonth)}&limit=1`, {
    method: 'GET'
  });
...
export async function getAnyBoardPostByLegacyId(legacyId: string): Promise<BoardSupabaseRow | null> {
  const rows = await supabaseRequest<BoardSupabaseRow[]>(`board_posts?select=*&legacy_id=eq.${encodeURIComponent(legacyId)}&limit=1`, {
    method: 'GET'
  });
...
export async function upsertBoardPost(input: BoardUpsertInput): Promise<BoardSupabaseRow> {
  const config = getSupabaseConfig();
  if (!config) {
    throw new Error('Supabase is not configured');
  }
  ...
  const rows = await response.json() as BoardSupabaseRow[];
  const row = rows[0];
  if (!row) {
...
export async function deleteBoardPostByLegacyId(legacyId: string): Promise<void> {
  const yearmonth = await getCurrentClanBattleYearMonth();
  const config = getSupabaseConfig();
  if (!config) {
    throw new Error('Supabase is not configured');
  ...
  const response = await fetch(
    `${getSupabaseTableEndpoint(config, 'board_posts')}?legacy_id=eq.${encodeURIComponent(legacyId)}&yearmonth=eq.${encodeURIComponent(yearmonth)}`,
    {
      method: 'DELETE',
      headers: {
        apikey: config.secretKey,
...
export function boardRowToArticle(row: BoardSupabaseRow): BoardArticleLike {
  const rawArticle = row.raw_article && typeof row.raw_article === 'object'
    ? row.raw_article as BoardArticleLike
    : {};
...
export function boardRowToListItem(row: BoardSupabaseRow): {
  id: string;
  mode: string;
  damage: number;
  battleTime: string;
...
export function normalizeBattleTimeSeconds(value: unknown): number {
  return parseBattleTimeToSeconds(value);
}

export function normalizeBattleDateIso(value: unknown): string {
