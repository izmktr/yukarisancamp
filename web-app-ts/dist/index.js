...
function canAttackClanBoss(bosslaps, attackBoss) {
...
function normalizeClanBossStateRow(raw) {
    if (!raw || typeof raw !== 'object') {
        return null;
    }
    const source = raw;
...
async function supabaseSelectClanBossStates(config, source, clanid, yearmonth) {
...
async function supabaseUpsertClanBossStateCurrentHp(config, source, clanid, yearmonth, bossIndex, currentHp, maxHp, updatedBy) {
    const endpointUrl = getSupabaseTableEndpoint(config, SUPABASE_CLAN_BOSS_STATE_TABLE);
    const query = new URLSearchParams({
...
async function resolveClanBossHpForDisplay(config, clan, clanBattleState) {
...
async function supabaseDeleteClanMember(config, source, clanId, membersource, memberid) {
    const endpointUrl = getSupabaseTableEndpoint(config, SUPABASE_CLAN_MEMBERS_TABLE);
    const query = new URLSearchParams({
        source: `eq.${source}`,
        clanid: `eq.${clanId}`,
...
async function supabaseStartClanMemberAttack(config, member, attackBoss, attackLap, sortie, carryOvertime = null) {
    const endpointUrl = getSupabaseTableEndpoint(config, SUPABASE_CLAN_MEMBERS_TABLE);
    const query = new URLSearchParams({
        source: `eq.${member.source}`,
        clanid: `eq.${member.clanid}`,
...
async function supabaseUpdateClanMemberAttackMessage(config, member, damage, attackMessage) {
    const endpointUrl = getSupabaseTableEndpoint(config, SUPABASE_CLAN_MEMBERS_TABLE);
    const query = new URLSearchParams({
        source: `eq.${member.source}`,
        clanid: `eq.${member.clanid}`,
...
async function supabaseDeleteAttackHistory(config, member, day, historyId) {
...
app.post('/api/clan/attack/carryover/start', ensureDiscordServerLinked, express_1.default.json(), async (req, res) => {
    const attackBoss = Number(req.body?.attackBoss);
    ...
    try {
        ...
        const attackLap = clan.bosslaps[attackBoss - 1];
        await supabaseStartClanMemberAttack(config, currentMember, attackBoss, attackLap, carryHistory.sortie, carryHistory.overtime);
        return res.json({ success: true });
    }
    catch (error) {
...
app.post('/api/clan/attack/message', ensureDiscordServerLinked, express_1.default.json(), async (req, res) => {
...
app.post('/api/clan/active-boss-hp/save', ensureDiscordServerLinked, express_1.default.json(), async (req, res) => {
    ...
    try {
        ...
        const resolvedMaxHp = targetState
            ...
            : Number.isFinite(settingMaxHp) && settingMaxHp >= 0
                ...
                : hp;
        await supabaseUpsertClanBossStateCurrentHp(config, clan.source, clan.clanid, clanBattleState.yearmonth, bossIndex, hp, resolvedMaxHp, discordId);
        return res.json({ success: true });
    }
    catch (error) {
...
app.post('/api/clan/attack-history/save', ensureDiscordServerLinked, express_1.default.json(), async (req, res) => {
    ...
    try {
        ...
        for (const history of histories) {
            ...
            const hasSameSortieHistory = currentHistories.some((item) => (item.id !== currentHistory.id && item.sortie === currentHistory.sortie));
...
app.post('/api/clan/attack-history/delete', ensureDiscordServerLinked, express_1.default.json(), async (req, res) => {
    ...
    try {
        ...
        const hasSameSortieHistory = attackHistories.some((history) => (history.id !== targetHistory.id && history.sortie === targetHistory.sortie));
